...
function operatorIsPostfix (operator) {
  return [')', ']', '}', '_', '!', '@:_', '_'].indexOf(operator) !== -1
}
...
exports.lex = function (includeWhitespace) {
  ...
  if (current === TYPE_OPERATOR) {
    ...
    if (last === TYPE_IDENTIFIER || last === TYPE_LETTER) {
      if (tokenText === '_') {
        tokenText = '@:_'
      }
    }
    if (last === TYPE_OPERATOR || last === null) {
      ...
      if (!last || !operatorIsPostfix(lastText)) {
  ...
  } else if (current === TYPE_IDENTIFIER && tokenText.length === 1) {
    current = TYPE_LETTER
  ...
  if (current === TYPE_LETTER) return TOKEN_LETTER
  return tokenText
}
