...
var DFunction = require('./commands/function')
...
function evaluate (expr) {
  ...
  if (headHeadDef && !headHeadDef.has(ATTRIBUTE.HOLD_FUNCTION)) {
    var fnSymbol = DFunction.symbol('Function')
    var headArgs = head.args()
    if (headArgs.some(function (arg) {
      var argHead = arg.head()
      if (argHead.def().invoke) {
        return true
      }
    })) {
      var x = new SymbolExpression('x')
      var newHead = new ApplicativeExpression(fnSymbol, [
        x,
        new ApplicativeExpression(headHead, headArgs.map(function (arg) {
          return evaluate(new ApplicativeExpression(arg, [x]))
        }))
      ])
      head = evaluate(newHead)
    }
  }
  ...
  if (headHeadDef && headHeadDef.invoke) {
    return headHeadDef.invoke(head, args)
  }

  headDef = head.def()
  if (headDef && headDef.run) {
    return headDef.run(args)
  }
